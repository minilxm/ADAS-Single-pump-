...
namespace AsyncSocket
{
    public class AsyncSocketUserToken
    {
        ...
        public long IP
        {
            get
            {
                if (m_connectSocket != null && m_connectSocket.RemoteEndPoint != null)
                {
                    IPEndPoint point = m_connectSocket.RemoteEndPoint as IPEndPoint;
                    byte[] ipBytes = point.Address.GetAddressBytes();
                    if (ipBytes.Length != 4)
                        return 0;
                    return (ipBytes[0] & 0xFF) + (ipBytes[1] << 8 & 0x00FF00) + (ipBytes[2] << 16 & 0x00FF0000) + (ipBytes[3] << 24 & 0x00FF000000);
                }
                else
                {
                    return 0;
        ...
        public string IPString
        {
            get
            {
                if (m_connectSocket != null && m_connectSocket.RemoteEndPoint != null)
                {
                    ...
                    if (ipBytes.Length != 4)
                        return "";
                    return string.Format("{0}.{1}.{2}.{3}", ipBytes[0], ipBytes[1], ipBytes[2], ipBytes[3]);
                }
                else
                {
                    return "";
